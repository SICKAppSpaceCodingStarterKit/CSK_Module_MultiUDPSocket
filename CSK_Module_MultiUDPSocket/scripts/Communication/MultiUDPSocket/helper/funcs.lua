...
funcs.json = require('Communication/MultiUDPSocket/helper/Json')
-- Default parameters for instances of module
funcs.defaultParameters = require('Communication/MultiUDPSocket/MultiUDPSocket_Parameters')
    ...
    local size = 0
      for key, value in pairs(content) do
        if list == 'commandList' then
          table.insert(commandList, {TriggerCommand = key, notifyEvent = 'CSK_MultiUDPSocket.' .. value})
