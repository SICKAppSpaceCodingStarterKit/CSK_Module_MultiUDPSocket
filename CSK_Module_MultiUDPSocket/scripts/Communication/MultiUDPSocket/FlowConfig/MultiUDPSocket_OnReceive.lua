-- Block namespace
local BLOCK_NAMESPACE = "MultiUDPSocket_FC.OnReceive"
...
local function register(handle, _ , callback)
  ...
  while true do
    local amount = CSK_MultiUDPSocket.getInstancesAmount()
  ...
  local function localCallback()
    ...
    if cbFunction ~= nil then
        Script.callFunction(cbFunction, 'CSK_MultiUDPSocket.OnNewData' .. tostring(instance))
    else
